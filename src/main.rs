...
enum POPCommand{
    ...
    Uidl(u32), User(String), Pass(String), Apop, Disconnect, Capa, Auth
}
enum SMTPCommand{
    Mailfrom(String), RcptTo(String), Data(String), Rset, Noop, Quit, Disconnect, Helo, Ehlo
}
...
fn main() {
    ...
    let account = (args.address.as_deref().unwrap_or("127.0.0.1"),
...
fn smtp_setup(){
    let args = Args::parse();
    let smtp_addr = (args.address.as_deref().unwrap_or("127.0.0.1"),
...
fn handle_pop_connection(args: &Args, mut stream: TcpStream, mut recent_id: String) -> Option<String> {
    ...
    for post in &timeline{
        ...
        if post["in_reply_to_id"] != Value::Null {
            message = message.in_reply_to(string_concat!(get_str(&post["in_reply_to_id"]), "@", account_domain));
        }
        if args.attachment{
            for attachment in attachments{
                message = message.binary_attachment(attachment.content_type, attachment.filename, attachment.data);
            }
        } else if args.inline {
            for attachment in attachments{
                message = message.binary_inline(attachment.content_type, attachment.filename, attachment.data);
            }
        }
...
fn handle_smtp_connection(mut stream: TcpStream, args: &Args){
    stream.write_all("220 hi welcome to chilis\r\n".as_bytes()).expect("Couldn't send welcome message");
    ...
    loop{
        match get_smtp_command(stream.try_clone().unwrap()){
            ...
            SMTPCommand::Mailfrom(addr) => {from = addr;}
            ...
            SMTPCommand::Rset => {from = "".to_string();}
...
fn get_pop_command(mut stream: TcpStream) -> POPCommand {
    ...
    loop {
        ...
        match split.next() {
            ...
            Some("AUTH") => {send_str!(stream, "-ERR Server does not support AUTH\r\n");return POPCommand::Auth},
            _ => (),
        }
    }
}
...
fn get_smtp_command(mut stream: TcpStream) -> SMTPCommand {
    ...
    loop {
        ...
        match tcp_read.read_until(b'\n', &mut cur_line_bytes){
            Ok(len) => if len == 0 {println!("Socket closed"); return SMTPCommand::Disconnect;},
            Err(err) => {println!("TCP error: {:?}", err); return SMTPCommand::Disconnect;}
        }
        ...
        if cur_line.starts_with("MAIL FROM:"){
            ...
            loop{
                ...
                if cur_line == ".\r\n"{break;} else {ret = string_concat!(ret, cur_line);}
